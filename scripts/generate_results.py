...
det_col = dict([(a, b)
                for a, b in zip(colorcode1.tech_code, colorcode1.tech_name)])
color_dict = dict([(a, b)
                   for a, b in zip(colorcode2.tech_name, colorcode2.colour)])
...
def gas_chart(Country):
    ...
    if len(gas_df.columns) == 1:
        ...
        pio.write_image(fig, '{}.png'.format(
            title+"-"+scenario), width=1300, height=800)
